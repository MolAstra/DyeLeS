...
if __name__ == "__main__":
    ...
    # 载入数据
    df_raw_path = data_dir / "coconut" / "coconut_csv-04-2025.csv"
    df_raw = pd.read_csv(df_raw_path)
    logger.info(f"Loaded {len(df_raw)} molecules.")
    df_raw = df_raw[df_raw["np_classifier_is_glycoside"] == 0]
    logger.info(f"Filtered {len(df_raw)} non-glycoside molecules.")
    
    standardizer = MoleculeStandardizer()

    # 处理分子
    sample_list = process_molecules(df_raw, standardizer, col_smiles="canonical_smiles")
    ...
    df.to_csv(processed_dir / "coconut.csv", index=False)
    logger.success(
        f"Saved {len(df)} processed molecules to {processed_dir / 'coconut.csv'}"
    )
