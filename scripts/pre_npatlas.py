...
if __name__ == "__main__":
    ...
    # 载入数据
    df_raw_path = data_dir / "npatlas" / "NPAtlas_download_2024_09.tsv"
    df_raw = pd.read_csv(df_raw_path, sep="\t")
    logger.info(f"Loaded {len(df_raw)} molecules.")

    standardizer = MoleculeStandardizer()

    # 处理分子
    sample_list = process_molecules(df_raw, standardizer, col_smiles="compound_smiles")
    ...
    df.to_csv(processed_dir / "npatlas.csv", index=False)
    logger.success(
        f"Saved {len(df)} processed molecules to {processed_dir / 'npatlas.csv'}"
    )
